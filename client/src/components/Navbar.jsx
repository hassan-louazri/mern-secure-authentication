...
const Navbar = () => {
    ...
    const { userData, serverUrl, setUserData, setIsLoggedIn } =
    ...
    const sendVerificationRequest = async () => {
        try {
            axios.defaults.withCredentials = true;
            const {data} = await axios.post(serverUrl + '/api/auth/send-verification-code');
                    ...
                    <div className="absolute hidden group-hover:block top-0 right-0 z-10 text-black rounded pt-12">
                        <ul className="list-none m-0 p-2 bg-gray-100 text-sm">
                            {!userData.isAccountVerified && (
                                <li onClick={sendVerificationRequest} className="py-1 px-2 hover:bg-gray-200 cursor-pointer">
                                    Verify email
                                </li>
                            )}
